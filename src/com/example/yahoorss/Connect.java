...
public class Connect {
	...
	/*
	 * rrayList <Front_v> front_view (String url) {
	 * 
	 * String results="failed";
	 * 
	 * String id= "" , name ="" ; HttpGet request = new HttpGet(url); ArrayList
	 * <Front_view> list = new ArrayList () ;
	 * 
	 * Front_view obj ;
	 * 
	 * 
	 * try { DefaultHttpClient client = new DefaultHttpClient(); HttpResponse
	 ...
	 * 
	 * JSONArray array = new JSONArray(results); for (int i = 0; i <
	 * array.length(); i++) {
	 * 
	 * obj = new Front_view () ; JSONObject row = array.getJSONObject(i); obj.id
	 * = row.getString("id"); obj.language = row.getString("language"); obj.body
	 * = row.getString("body") ; obj.head = row.getString("head") ; obj.priority
	 * = row.getString("priority") ; obj.image_url = row.getString("image_url")
	 * ; obj.category = row.getString("category") ;
	 * 
	 * obj.timestamp = row.getString("timestamp") ; list.add(obj); }
	 * 
	 * } catch (Exception e) { e.printStackTrace(); return null ; }
	 * 
	 * if(results.equals("failed")==true) { return null ; } else { return list ;
	 * 
	 * }
	 * 
	 * }
