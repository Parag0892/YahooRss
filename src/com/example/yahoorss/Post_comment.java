...

{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		post.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				// AlertDialog.Builder(Post_comment.this);
					...
					public void onClick(View v) {
						...
						if (comment.getText().toString().length() == 0
							...
							Toast.makeText(getApplicationContext(),
									"Please fill the entries carefully", 2000)
									.show();
							count = 0;
						} else {
	...
	private class SomeTask extends
			AsyncTask<String, Void, ArrayList<CommentFields>> {
		...
		protected ArrayList<CommentFields> doInBackground(String... params) {
			if (count == 0) {
				...
				try {
					...
					li = ob.retrieveComments("http://obscure-escarpment-4510.herokuapp.com/retrieve_comments/?foreign_key="
							+ link);
				} catch (Exception e) {
				...
				try {
					...
					HttpGet request = new HttpGet(
							"http://obscure-escarpment-4510.herokuapp.com/post_comment/?name="
									+ common[0].replace(" ", "_").replace("\n",
											":")
									+ "&comment="
									+ common[1].replace(" ", "_").replace("\n",
											":")
									+ "&location="
									+ common[2].replace(" ", "_").replace("\n",
											":") + "&foreign_key=" + link);
			...
			if (count == 0) {
				...
				if (list != null) {
					...
					for (CommentFields n : list) {
						...
						paths_1.add(n.comment.toString() + "\n" + n.name + ","
								+ n.location);
					}
					comment_s.setAdapter(new MyAdapter(Post_comment.this,
							R.layout.photos_1, paths_1));
	...
	public class MyAdapter extends ArrayAdapter<String> {
		...
		public View getCustomView(int position, View convertView,
			...
			label1.setText(li.get(position).comment.toString()
					.replace("_", " ").replace(":", "\n"));
			label2.setText(li.get(position).name.toString().replace("_", " ")
					.replace(":", "\n")
					+ ","
					+ li.get(position).location.toString().replace("_", " ")
							.replace(":", "\n"));
